function [output] = data_process(data,method)
%一些数据处理方法
global imgrow
global imgcol
imgrow = 112;
...
elseif strcmp(method,'norm_type')
    [m,n] = size(data);
    output = zeros(m,n);
    mean_type = zeros(m/5,n);
    for i=1:m/5
        mean_type(i,:) = sum(data(i*5-4:i*5,:))/5 ;
    end
    for i = 1:m
        id = floor((i-1) / 5) + 1;
        output(i,:) = data(i,:) - mean_type(id,:);
    end
elseif strcmp(method,'norm_all')
    [m,n] = size(data);
    x = mean(data,1);
    output = data - ones(m,1)*x;
elseif strcmp(method,'wavelet')
    [m,n] = size(data);
    tmp_img = reshape(data(1,:),[imgrow,imgcol]);
    [tmp_cA,~,~,~] = dwt2(tmp_img,'haar')
    output = zeros(m,size(tmp_cA,1)*size(tmp_cA,2));
    for i = 1:m                
        tmp_img = reshape(data(i,:),[imgrow,imgcol]);
        [cA,cH,cV,cD]=dwt2(tmp_img,'haar');
        output(i,:) = cA(:)';
    end
elseif strcmp(method,'lbp')
    [m,n] = size(data);        
    output = zeros(m,imgrow*imgcol);
    for i = 1:m
        tmp_data = reshape(data(i,:),[imgrow,imgcol]);
        tmp_output = convolution(tmp_data);
        output(i,:) = tmp_output(:)';
    end    
elseif strcmp(method,'uni_lbp')
    [m,n] = size(data);        
    output = zeros(m,imgrow*imgcol);
    for i = 1:m
        tmp_data = reshape(data(i,:),[imgrow,imgcol]);
